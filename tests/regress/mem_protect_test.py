...
class TestMemProtect(unittest.TestCase):
  ...
  def test_perm_protect_slicing_right_border(self):
    self.uc.mem_map(self.ADDRESS, 0xc000, UC_PROT_NONE)
    self.uc.mem_protect(self.ADDRESS, 0xb410, UC_PROT_EXEC)
    regs = self.get_mem_perm_regions()
    self.assertEqual(len(regs), 2)

    self.uc.mem_map(self.ADDRESS + 0xc000, 0x3000, UC_PROT_NONE)
    regs = self.get_mem_perm_regions()
    self.assertEqual(len(regs), 3)

    self.uc.mem_protect(self.ADDRESS + 0xb410, 0x38cb, UC_PROT_WRITE)
    regs = self.get_mem_perm_regions()
    self.assertEqual(len(regs), 4)
    for i, (b, e, p) in enumerate([(self.ADDRESS, self.ADDRESS + 0xb40f, UC_PROT_EXEC),
                                 (self.ADDRESS + 0xb410, self.ADDRESS + 0xbfff, UC_PROT_WRITE),
                                 (self.ADDRESS + 0xc000, self.ADDRESS + 0xecda, UC_PROT_WRITE),
                                 (self.ADDRESS + 0xecdb, self.ADDRESS + 0xefff, UC_PROT_NONE)
                                 ]):
      self.assertEqual(regs[i].begin, b)
      self.assertEqual(regs[i].end, e)
      self.assertEqual(regs[i].perm, p)

  def test_perm_protect_slicing_border_access(self):
    self.uc.mem_map(0x20000000, 0x1000, UC_PROT_NONE)
    self.uc.mem_protect(0x20000000, 0x210, UC_PROT_EXEC)
    regs = self.get_mem_perm_regions()
    self.assertEqual(len(regs), 2)

    self.uc.mem_map(0x20001000, 0x4000, UC_PROT_NONE)
    regs = self.get_mem_perm_regions()
    self.assertEqual(len(regs), 3)

    self.uc.mem_protect(0x20000400, 0x4530, UC_PROT_WRITE)
    regs = self.get_mem_perm_regions()
    self.assertEqual(len(regs), 5)
    for i, (b, e, p) in enumerate([(0x20000000, 0x20000000 + 0x20f, UC_PROT_EXEC),
                                 (0x20000000 + 0x210, 0x20000000 + 0x3ff, UC_PROT_NONE),
                                 (0x20000000 + 0x400, 0x20000000 + 0xfff, UC_PROT_WRITE),
                                 (0x20000000 + 0x1000, 0x20000000 + 0x492f, UC_PROT_WRITE),
                                 (0x20000000 + 0x4930, 0x20000000 + 0x4fff, UC_PROT_NONE)
                                 ]):
      self.assertEqual(regs[i].begin, b)
      self.assertEqual(regs[i].end, e)
      self.assertEqual(regs[i].perm, p)
    ...
    self.uc.mem_write(0x20000fff, self.ARM_CODE_LDR)
